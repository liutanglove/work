	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				title: "文件上传",
				isUpload: true,
				browserUrl: "/shop_ds_ui/speci/browser",
				uploadUrl:  "/shop_ds_ui/speci/imgUpload",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					if (settings.isUpload) {
						...
						$browserUploadInput = $('<input type="file" name="source" \/>').appendTo($browserUploadButton);
					}
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = "/shop_ds_ui/back/img/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl ="/shop_ds_ui/back/img/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
